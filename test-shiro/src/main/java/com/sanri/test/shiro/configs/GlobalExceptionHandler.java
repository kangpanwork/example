...
import com.sanri.test.shiro.configs.exception.BusinessException;
import com.sanri.test.shiro.configs.exception.RemoteException;
import com.sanri.test.shiro.configs.exception.SystemMessage;
import com.sanri.test.shiro.configs.exception.ToolException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseDto authenticationException(AuthenticationException e){
        log.error("用户名密码错误:{}",e.getMessage(),e);
        return SystemMessage.AUTHENTICATION_DENIED.exception().getResponseDto();
    }

    @ExceptionHandler(AuthorizationException.class)
    public ResponseDto authorizationException(AuthorizationException e){
        log.error("用户无权限:{}",e.getMessage(),e);
        return SystemMessage.AUTHORIZATION_DENIED.exception().getResponseDto();
    }

    @ExceptionHandler(UnauthorizedException.class)
    public ResponseDto unauthorizedException(UnauthorizedException e){
        log.error("用户无权限:{}",e.getMessage(),e);
        return SystemMessage.AUTHORIZATION_DENIED2.exception(e.getMessage()).getResponseDto();
    }
}
