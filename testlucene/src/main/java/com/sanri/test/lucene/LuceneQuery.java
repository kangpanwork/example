...
public class LuceneQuery {
	...
	public static Version V = Version.LUCENE_35;				//lucene 版本
	...
	public void createIndex(boolean create){
		...
		try {
			...
			for (IndexData indexData : loadData) {
				...
				//加入一个评分域
				document.add(new NumericField("score",Field.Store.NO,true).setFloatValue(300f));
...
//		phraseQuery.add(new Term("content","pingpeng"));
		//第一个Term
	...
	public void parserQuery(){
		try {
			...
			Query query = queryParser.parse("football");
...
//			queryParser.setAllowLeadingWildcard(true);
	 ...
	 * @param query
	 */
	public void searchAndShowResult(Query query){
		try {
			IndexSearcher searcher = getSearcher();
	 ...
	 * @param searcher
	 */
	public void printDocs(TopDocs topDocs, IndexSearcher searcher) {
			...
			for (ScoreDoc scoreDoc:topDocs.scoreDocs) {
				...
				System.out.println("id:"+doc.get("id")+",name:"+doc.get("name")+"" +
						",email:"+doc.get("email")+",attachs:"+doc.get("attachs")+"" +
								",date:"+DateFormatUtils.format(Long.parseLong(doc.get("date")),"yyyy-MM-dd")+",score:["+scoreDoc.score+"]");
			}
		} catch (CorruptIndexException e) {
			e.printStackTrace();
		} catch (IOException e) {
